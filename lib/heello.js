...
function heelloAction(api, controller, action, details) {
	...
	var ret = function(params, fn) {
		...
		try {
			...
			if(this.needsAuth && self.api.accessToken === null)
			...
			// <https://github.com/Heello/Issues/issues/10>
			if(this.needsAuth) {
			...
			self.api._request(options, function(err, json, res) {
				if(err && err.message === ETOKENEXPIRED) {
					// swap out with new token, try again pls
					self.api.refreshTokens(function() {
						options.queryData['access_token'] = self.api.accessToken
						self.api._request(options, fn)
					})
				} else {
					fn(err, json, res)
				}
